...
if [ $retVal -eq 0 ]; then
  ...
  docker cp $INPUT_FILE mediapipe:/root/input.mp4
  docker exec mediapipe ${DOCKER_MEDIAPIPE_DIR}/run_yt8m_feature_extraction.sh /root/input.mp4 /root/yt8m_feature_output
  docker cp mediapipe:/root/yt8m_feature_output ${OUTPUT_DIR}
  exit 0
fi
...
mkdir -p ${OUTPUT_DIR}
cp /tmp/mediapipe/metadata.pb ${OUTPUT_DIR}
cp /tmp/mediapipe/features.pb ${OUTPUT_DIR}
